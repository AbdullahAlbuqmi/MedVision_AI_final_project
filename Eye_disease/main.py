...
MODEL_PATH = "eye_disease_finetuned.h5"
...
def load_model_with_fix(model_path):
    ...
    try:
        ...
        if "shape mismatch" in str(e).lower():
            ...
            try:
                ...
                # Method 3: Try loading with custom objects
                try:
                    model = load_model(
                        model_path, 
                        compile=False,
...
def build_and_load_model(model_path):
    ...
    try:
        ...
        num_classes = 3  # Cataracts, Normal_Eyes, Uveitis
        ...
        x = Dropout(0.25)(x)  # Note: 0.25 dropout as in your training
...
# Class labels for eye diseases
class_labels = ["Cataracts", "Normal_Eyes", "Uveitis"]
print(f"Class labels: {class_labels}")

@app.get("/")
def home():
    return {
        "message": "Eye Disease Classifier API is running",
        "status": "healthy",
        "model_loaded": True,
        "endpoints": {
            "health": "/health",
        ...
        "instructions": "Use /predict endpoint with POST method to upload an image for classification"
    }
...
async def predict(file: UploadFile = File(...)):
    ...
    Expected diseases: Cataracts, Normal_Eyes, Uveitis
    """
    try:
        # Validate file type
